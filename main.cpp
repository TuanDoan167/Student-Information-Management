    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            // TODO: Add functionality here
                ...
                std::string studentnumber = parts.at(1);
                if(student_numbers.find(studentnumber)==student_numbers.end()){
                    ...
                    std::cout<< std::endl;
                    if(not is_valid_phone_number(newphonenumber)){
                        continue;
                    }
                    else{
                        std::vector<std::string>tobechanged;
                        student_numbers.find(studentnumber)->second->phone_number=newphonenumber;
                        for(auto entry : user_ids){
                            if( entry.second->student_number==studentnumber){
                                entry.second->phone_number=newphonenumber;
                            }
                            std::string line=entry.second->student_number+";"+entry.first+";"+entry.second->name+";"+entry.second->phone_number+
                                    ";"+entry.second->email+";"+entry.second->skype;
                            tobechanged.push_back(line);
                        }
                        std::fstream myfile;
                        myfile.open (file_name,std::fstream::out | std::fstream::trunc);
                        std::vector<std::string>::size_type n=0;
                        while(n<tobechanged.size()){
                            myfile<<tobechanged.at(n)<<std::endl;
                            ++n;
                        }
                        myfile.close();
                    }
